...
class MultiProcessQueue{
	...
	public function __construct($config) {
		...
		foreach($connections['config'] as $connectionName => $connectionConfig){
			\Shimasuo\Connection\Connection::setConnectionConfig(
		...
		$this->taskQueue = new \Shimasuo\Queue\TaskQueue(
			$connections['task'],
			"{$keyPrefix}SQ_TASK_LIST"
		);
			
		$this->workerQueue = new \Shimasuo\Queue\Queue(
			$connections['worker'],
